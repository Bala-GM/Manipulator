...
# Your existing DataFrame initialization
df = pd.DataFrame(columns=['Type', 'Capacitance', 'Tolerance', 'Unit', 'Voltage', 'Impedance', 'Dimensions'])
...
def extract_capacitor_info(component_line):
    # Check if the line contains "CAP"
    if "CAP" in component_line:
        # Extract relevant information using regular expressions
        #capacitance_match = re.search(r'(\d+)([UPpnuμmkKM]F)', component_line)
        ...
        if capacitance_match:
            capacitance = capacitance_match.group(1)
            unit = capacitance_match.group(2)

        if tolerance_match:
            tolerance = tolerance_match.group(1)

        return capacitance, tolerance, unit
    else:
...
for component_line in component_lines:
    component_type = "Cap"
    capacitance, tolerance, unit = extract_capacitor_info(component_line)

    # Extract other relevant information based on the structure of your data
    # (You need to customize the extraction based on the actual structure of your data)

    new_row = {'Type': component_type, 'Capacitance': capacitance, 'Tolerance': tolerance, 'Unit': unit, 'Voltage': 'Voltage_Info', 'Impedance': 'Impedance_Info', 'Dimensions': 'Dimensions_Info'}
